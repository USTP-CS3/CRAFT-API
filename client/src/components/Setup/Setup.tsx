...
import { Stepper, Button, Group, Container, Flex, Text, Checkbox, Space, Anchor, rem } from '@mantine/core';
...
function Setup() {
  ...
  const stepper = (
    <>
      ...
      <Button variant="light" size="sm" mb={20} ml={60} leftSection={<IconArrowLeft size={14}/>} onClick={() => Google.logout()}>Sign out</Button>
      <Stepper active={active} pl={30} pr={30} ml={isMobile?0:30} mb={isMobile?80:30} radius='md' onStepClick={setActive} orientation={isMobile?"horizontal":"vertical"}>
      ...
      <Container>
        {active === 1 && 
          <Container>
            ...
            <Space w={10} />
            <Text fz="md" c="dimmed">
              I agree to the <Anchor href="https://mantine.dev/" target="_blank" underline="hover"> Terms and Conditions</Anchor>
            </Text>
            </Flex>
          </Container>}
  ...
  return (
    <ResizeProvider callback={handleResize}>
      ...
      <Container size='md'>
        {
          !isMobile &&
          <Flex align={'center'} justify={'space-between'} style={{minHeight: '100vh', width: '100%'}}>
            {stepper}
          </Flex>
        }
